...
#
# 放用户目录，**不进版本库，但会打进安装包** —— 这是权衡之后定的：要的是
# 「GitHub 上发出去的安装包装完和本机一致」。于是仓库里只有读取它的 `voice.py`，
# 音频本身被 `/voice/` 忽略掉，再由 `installer\taffy-pet.iss` 收进安装程序。
# 剪出这些片段的构建脚本在 `_work\` 下（同样被忽略，不随包发布）。
#
# 少了它不影响任何别的东西：她不出声，其余功能一律照常（voice.py 拿不到目录
# 就是空库，菜单里「说话出声」那一项照旧在）。
VOICE_DIR = DATA_DIR / "voice"

# 跳舞精灵表：一张所有帧横排的 PNG + 一份 dance.json。
#
# **和语音库同一个待遇**：不进版本库，但进安装包。它是从视频素材里逐帧抠出来、
# 再挑出干净帧拼成的 —— 属于「第三方的画面」那一类，不是这个仓库自己的东西。
# 仓库里只留读取它的 `pet.py`；抠帧和安装的脚本在 `_work\` 下（被忽略，不发布）。
#
