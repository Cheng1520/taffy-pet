...
- **窗口打开时轻微淡入（spec:213）**：**完全没实现**。`showEvent` 只刷标题栏，没有
  `QPropertyAnimation`，也没有任何透明度动画。这一轮终审才认账。
- **正文 10.5pt / 行高 1.5（spec:205）**：实装是 `QFont("Microsoft YaHei UI", 10)`，
  行高没有任何对应实现 —— Qt 的 QSS 不支持 `line-height`，要做得自己接
  `QTextBlockFormat` 或者用富文本，不划算。这条同样是这一轮才写进偏差表。
- **`assets/persona.md` 的体积（plan:109「目标 1.5KB 以内」）**：实测 **1797 字节**，
  超出软指标约 17%。属于「既然写了就该认账」，不影响功能。

**被推翻 / 被升级的旧裁定（终审复审的结论，别再按旧的说法办）**

- **`_append_widget` 里那个全量 `_apply_bubble_width()`（原写「实测 0.47s、可接受、不改」）
  —— 推翻。** 0.47s 是**关掉**那次刷新之后的数，主导项整个漏掉了。带上是 1.675s（原生），
  「历史攒满 200 条之后每次开窗」都要付一遍 2~5 秒的白屏。现已拆成
  `_apply_width_to(bubble)`（append 时只设这一个）+ `_apply_bubble_width()`（只在
  `resizeEvent` 里全量）。**别再引用那个 0.47s。**
- **配色（原按「风格问题、先记着」处理）—— 升级为必须守的约束。** Global Constraints
  明文写着「沿用 `toast.py` 的 `BG / BORDER / TEXT / DIM`，不要另起一套」，而实现里一次都
  没 import toast，手抄的那份**已经错了两个值**（窗口底色 `#FFFBF8` vs `BG`、系统提示
  `#96848C` vs `DIM`）。现已改成从 toast 引用并把 QSS 字面量提成模块常量。
